...
const MOCK: Order[] = [
  { id: "1", orderNumber: 1001, total: 18990, status: "DELIVERED", paymentStatus: "APPROVED", paymentMethod: "PIX", trackingCode: "BR123456789", createdAt: new Date("2025-10-15"), userName: "Lucas Fernandes", userEmail: "lucas@email.com", itemCount: 2 },
  { id: "2", orderNumber: 1002, total: 35480, status: "SHIPPED", paymentStatus: "APPROVED", paymentMethod: "CREDIT", trackingCode: "BR987654321", createdAt: new Date("2025-10-16"), userName: "Mariana Costa", userEmail: "mariana@email.com", itemCount: 3 },
  { id: "3", orderNumber: 1003, total: 8990, status: "PROCESSING", paymentStatus: "APPROVED", paymentMethod: "PIX", trackingCode: null, createdAt: new Date("2025-10-17"), userName: "Rafael Souza", userEmail: "rafael@email.com", itemCount: 1 },
  { id: "4", orderNumber: 1004, total: 12990, status: "PENDING", paymentStatus: "PENDING", paymentMethod: null, trackingCode: null, createdAt: new Date("2025-10-18"), userName: "Camila Rodrigues", userEmail: "camila@email.com", itemCount: 1 },
  { id: "5", orderNumber: 1005, total: 6990, status: "PAID", paymentStatus: "APPROVED", paymentMethod: "BOLETO", trackingCode: null, createdAt: new Date("2025-10-19"), userName: "Pedro Lima", userEmail: "pedro@email.com", itemCount: 1 },
  { id: "6", orderNumber: 1006, total: 22980, status: "CANCELLED", paymentStatus: "REFUNDED", paymentMethod: "CREDIT", trackingCode: null, createdAt: new Date("2025-10-20"), userName: "Ana Santos", userEmail: "ana@email.com", itemCount: 2 },
];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-colors border ${statusFilter === "ALL" ? "bg-[#f5f5f5] text-[#1a1a1a] border-[#aaaaaa]" : "bg-white text-[#999] border-[#e8e8e8] hover:border-[#aaaaaa] hover:text-[#1a1a1a]"}`}
        >
          Todos ({orders.length})
        </button>
        {statusCounts.filter((s) => s.count > 0).map(({ key, label, count }) => (
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-colors border ${statusFilter === key ? `${ORDER_STATUS_COLORS[key]} border-current` : "bg-white text-[#999] border-[#e8e8e8] hover:border-[#aaaaaa] hover:text-[#1a1a1a]"}`}
          >
            {label} ({count})
          </button>
        ))}
              ...
              {filtered.map((o) => (
                <tr key={o.id} className="border-b border-[#f8f7fb] hover:bg-[#faf9fd] transition-colors">
                  <td className="px-5 py-3.5">
                    <button onClick={() => { setSelectedOrder(o); setTracking(o.trackingCode ?? ""); }} className="text-sm font-semibold text-[#1a1a1a] hover:underline">
                      #{o.orderNumber}
                    </button>
                  ...
                  <td className="px-5 py-3.5">
                    <span className="text-xs text-[#999]">{o.paymentMethod ?? "—"}</span>
                  </td>
                  <td className="px-5 py-3.5">
                      ...
                      {o.status !== "DELIVERED" && o.status !== "CANCELLED" && (
                        <button
                          onClick={() => {
                            ...
                            if (idx < statusFlow.length - 1) updateStatus(o.id, statusFlow[idx + 1]);
                          }}
        ...
        {selectedOrder && (
          <>
            ...
            <motion.aside
              ...
              className="fixed right-0 top-0 h-full w-full max-w-md bg-white border-l border-[#f0f0f0] z-50 flex flex-col shadow-[-8px_0_32px_rgba(98,41,157,0.08)] overflow-y-auto"
            >
              <div className="flex items-center justify-between p-5 border-b border-[#f0f0f0] sticky top-0 bg-white z-10">
                ...
                <button onClick={() => setSelectedOrder(null)} className="p-2 text-[#bbb] hover:text-[#1a1a1a] rounded-lg hover:bg-[#f5f5f5] transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${
                          selectedOrder.status === s
                            ? `${ORDER_STATUS_COLORS[s]} border-current`
                            : "border-[#e8e8e8] text-[#999] hover:border-[#aaaaaa] hover:text-[#1a1a1a]"
                        }`}
                      >
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${
                        selectedOrder.status === "CANCELLED"
                          ...
                          : "border-[#e8e8e8] text-[#999] hover:border-red-200 hover:text-red-400"
                      }`}
                    >
                      Cancelado
                    </button>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-[#faf9fd] border border-[#e8e8e8] focus:border-[#aaaaaa] rounded-xl px-4 py-2.5 text-sm text-[#111] outline-none transition-colors placeholder-[#bbb]"
                    />
                    <button
                      onClick={() => saveTracking(selectedOrder.id)}
                      className="px-4 py-2.5 bg-[#f5f5f5] border border-[#aaaaaa] text-[#1a1a1a] rounded-xl hover:bg-[#efefef] transition-colors"
                    >
                      <Check className="w-4 h-4" />
                    </button>
                  </div>
                  {selectedOrder.trackingCode && (
                    <p className="text-xs text-[#1a1a1a] mt-1.5 font-medium">✓ {selectedOrder.trackingCode}</p>
                  )}
                </div>
